...
import { FiClipboard, FiGrid, FiMic, FiTruck, FiUsers, FiX } from 'react-icons/fi';
        ...
        <tbody>
          {rows.map(row => (
            <tr key={row.id}>
              ...
              <td style={S.td}>
                {onDelete && (
                  <button onClick={() => onDelete(row.id)} style={{ background:'none', border:'none', color:'#dc2626', cursor:'pointer', fontSize:13 }}><FiX /></button>
                )}
              </td>
            </tr>
          ))}
...
function AuditLog() {
  ...
  return (
    <div>
      <div style={{ fontSize:14, fontWeight:700, textTransform:'uppercase', letterSpacing:'1px', color:'#333', marginBottom:12 }}><FiClipboard style={{ verticalAlign:'middle', marginRight:6 }} />Audit Log</div>
...
export default function AdminCostData() {
  ...
  return (
    <div style={{ fontFamily:"'Jost',sans-serif" }}>
      <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:28, color:'#1a0a0a', margin:'0 0 6px' }}><FiGrid style={{ verticalAlign:'middle', marginRight:8 }} />Cost Data Management</h1>
      ...
      {loading && activeTab !== 'audit' ? (
        ...
        <>
          {activeTab === 'artists' && (
            <CostTable
              title="Artists & Entertainment" icon={<FiMic />}
              rows={artists}
              columns={[
          ...
          {activeTab === 'cities' && (
            <CostTable
              title="City Cost Multipliers" icon={<FiGrid />}
              rows={cities}
              columns={[
          ...
          {activeTab === 'meals' && (
            <CostTable
              title="Meal Types (per head)" icon={<FiUsers />}
              rows={meals}
              columns={[
          ...
          {activeTab === 'bar' && (
            <CostTable
              title="Bar Tiers (per head)" icon={<FiGrid />}
              rows={barTiers}
              columns={[
          ...
          {activeTab === 'counters' && (
            <CostTable
              title="Specialty Counters" icon={<FiGrid />}
              rows={counters}
              columns={[
          ...
          {activeTab === 'logistics' && logistics && (
            <CostTable
              title="Logistics Rates" icon={<FiTruck />}
              rows={[logistics]}
              columns={[
          ...
          {activeTab === 'sundries' && sundries && (
            <CostTable
              title="Sundries & Extras Config" icon={<FiGrid />}
              rows={[sundries]}
              columns={[
